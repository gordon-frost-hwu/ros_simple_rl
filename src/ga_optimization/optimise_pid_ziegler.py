...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--validate", type=str, nargs=1, default="", help="Validate a learning run by removing exploration"
    )
    ...
    if args.validate == "":
        ...
        print(args.validate)
        result_dir = dir_manager.get_validation_name(args.validate[0])
        print("validation result dir name: {0}".format(result_dir))
        dir_manager["run"] = result_dir
        process = PilotPidProcess(ros_env, result_dir)

        k_crit = np.loadtxt("{0}/gains.csv".format(args.validate[0]))[0]
        p_crit = 6.5    #8.39   # seconds
        ...
        k_i_p = 0.0
        k_i_pi = (1.0 / 1.2) * p_crit
        k_i_pid = 0.5 * p_crit

        k_d_p = 0.0
        k_d_pi = 0.0
        k_d_pid = 0.125 * p_crit

        crit_gains = [k_crit, 0, 0]
        process.get_response(0, crit_gains)

        p_gains = [k_p_p, k_i_p, k_d_p]
        process.get_response(1, p_gains)

        pi_gains = [k_p_pi, k_i_pi, k_d_pi]
        process.get_response(2, pi_gains)

        pid_gains = [k_p_pid, k_i_pid, k_d_pid]
        process.get_response(3, pid_gains)
