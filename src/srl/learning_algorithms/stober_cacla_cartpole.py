...
from srl.environments.stober_cartpole import CartPole
...
if __name__ == '__main__':
    ...
    for run in range(CONFIG["num_runs"]):
        ...
        cartpole_environment = CartPole(x=0.05, xdot=0.0, theta=-0.05, thetadot=0.0)
        ...
        for episode_number in range(CONFIG["num_episodes"]):
            ...
            # if episode_number > 50 and CONFIG["exploration_sigma"] - CONFIG["exploration_decay"] > 0.0:
            #     CONFIG["exploration_sigma"] -= CONFIG["exploration_decay"]
            ...
            for step_number in range(CONFIG["max_num_steps"]):
                ...
                # action_t_deterministic = policy.computeOutput(basis_functions.computeFeatures(state_t.values()))
                ...
                print("SIGMA: {0}".format(CONFIG["exploration_sigma"]))
                ...
                if td_error > 0.0 and episode_number > 1:
                    ...
                    # old_action = policy.computeOutput(basis_functions.computeFeatures(state_t.values()))
                    ...
                    # print("RAAARRRWW: {0}".format(basis_functions.computeFeatures(state_t.values())))
                    ...
                    if policy.name == "scratch_ann":
                        ...
                        new_action = policy.computeOutput(basis_functions.computeFeatures(state_t.values()))
                    else:
                        print("Give approximator a name attribute!!!")
                        exit(0)
                ...
                if cartpole_environment.failure():
