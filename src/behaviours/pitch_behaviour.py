...
GAUSSIAN_VARIANCE = 0.4     # Open parameter which has a MASSIVE effect on resulting velocity request
...
class PitchBehaviour(Behaviour):
    ...
    def navCallback(self, msg):
        ...
        """
        self._nav = msg.orientation.pitch
    ...
    def doWork(self):
        ...
        if self.goal_position is not None and self._nav is not None:
            ...
            desired_velocity = 1.0 - gaussian1D(self._nav, self.goal_position[4],
                                          1.0, self._params['gaussian_variance'])

            print("Resulting Surge Velocity:  {0}".format(desired_velocity))

            # Limit the velocities by the maximum possible for the vehicle if required.
            scaled_velocity = np.array([0, 0, 0, 0, desired_velocity, 0]) * self.MAX_SPEED
