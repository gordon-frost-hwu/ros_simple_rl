...
YAW = 2
# was 1.0
GAUSSIAN_VARIANCE_DEFAULT = 0.0     # Open parameter which has a MASSIVE effect on resulting velocity request
    ...
    def doWork(self):
        ...
        if self.goal_position is not None and self._nav is not None:
            ...
            if goal_angle_wrt_vehicle_heading > math.pi:
                goal_angle_wrt_vehicle_heading = -(2.0 * math.pi - goal_angle_wrt_vehicle_heading)
            ...
            #     desired_velocity = 1.0 - gaussian1D(self.raw_angle_to_wp, self._params['gaussian_mean'],
            ...
            # if self.raw_angle_to_wp > self._params['gaussian_mean']:
            #     desired_velocity = -desired_velocity

            # Limit the velocities by the maximum possible for the vehicle if required.
            scaled_velocity = np.clip(np.array([0, 0, 0, 0, 0, self._params['gaussian_variance']]), -self.MAX_SPEED, self.MAX_SPEED)
