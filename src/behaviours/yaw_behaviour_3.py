...
class YawBehaviour(Behaviour):
    ...
    def __init__(self):
        ...
        self._params['gaussian_mean'] = GAUSSIAN_MEAN
    ...
    def doWork(self):
        ...
        if self.goal_position is not None and self._nav is not None:
            ...
            if self.distance_to_wp < GOAL_PRECISION:
                self.at_goal_translation = True
            else:
                self.at_goal_translation = False
            ...
            # TODO: add condition for if not IP
            # if not self.at_goal_translation:
            ...
            #         desired_velocity = -(1 - gaussian1D(goal_angle_wrt_vehicle_heading, 0.0, 1.0, self._params['gaussian_variance']))
            # else:
            #    desired_velocity = -(1 - gaussian1D(self._nav[YAW], goal[YAW], 1.0, self._params['gaussian_variance']))
            if self._params['reverseAction']:
                desired_velocity = 1 - gaussian1D(self.raw_angle_to_ip, self._params['gaussian_mean'], 1.0, self._params['gaussian_variance'])
            else:
                # print("IM REVERSING MY OUTPUT --- LOOK OUT!!!")
                desired_velocity = -(1 - gaussian1D(self.raw_angle_to_ip, self._params['gaussian_mean'], 1.0, self._params['gaussian_variance']))
