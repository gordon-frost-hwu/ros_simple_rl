...
class TranslationalBehaviour(Behaviour):
    ...
    def navCallback(self, msg):
        ...
        """
        self._nav = (msg.position.north, msg.position.east, msg.position.depth)
        self._nav_orientation = (msg.orientation.pitch, msg.orientation.yaw)
    ...
    def doWork(self):
        ...
        if self.ip_position is not None and self._nav is not None:
            ...
            print("Pitch: {0}".format(self._nav_orientation[0]))
            pos_err = self.ip_position[-2] - self._nav_orientation[0]

            pid_output = self.pid.GenOut(pos_err)
            print("Pitch PID request: {0}".format(pid_output))
            # Limit the velocities by the maximum possible for the vehicle if required.
            scaled_velocity = np.array([0, 0, 0, 0, pid_output, 0]) #* self.LIMITED_SPEED
...
if __name__ == '__main__':
    ...
    rospy.init_node('pitch_behaviour')
