...
class TranslationalBehaviour(Behaviour):
    ...
    def __init__(self):
        ...
        self.pid = PID(gains=self.pid_coeffs["depth"])
    ...
    def doWork(self):
        ...
        if self.ip_position is not None and self._nav is not None:
            ...
            print("Depth: {0}".format(self._nav[-1]))
            pos_err = self.ip_position[2] - self._nav[-1]

            pid_output = self.pid.GenOut(pos_err)
            # if abs(self.ip_position[2]) < 0.1 and abs(self._nav[-1]) < 0.1:
            #     pid_output = 0.0
            print("Depth PID request: {0}".format(pid_output))
            # Limit the velocities by the maximum possible for the vehicle if required.
            scaled_velocity = np.array([0, 0, pid_output, 0, 0, 0]) #* self.LIMITED_SPEED
