        ...
        if self.ip_position is not None and self._nav is not None:
            ...
            if self.distance_to_ip < 1.0:
                if self.ip_position[-1] >= 0.0:
                    pos_err = (self.ip_position[-1] - abs(self.nav_yaw)) * np.sign(self.nav_yaw)
                else:
                    pos_err = (self.ip_position[-1] - self.nav_yaw)# * np.sign(self.nav_yaw)
            else:
            ...
            scaled_velocity = np.array([0, 0, 0, 0, 0, pid_output]) * self.LIMITED_SPEED
