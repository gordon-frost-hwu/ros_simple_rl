...
class TranslationalBehaviour(Behaviour):
    ...
    def __init__(self):
        ...
        self.pid = PID(gains=self.pid_coeffs["surge"])
    ...
    def doWork(self):
        ...
        if self.ip_position is not None and self._nav is not None:
            ...
            # pos_err = self.goal_position[0] +
            pos_err = self.distance_to_ip
            if abs(self.raw_angle_to_ip) > 1.57:
                pos_err = -pos_err
            pid_output = self.pid.GenOut(pos_err)
            # Limit the velocities by the maximum possible for the vehicle if required.
            scaled_velocity = np.array([pid_output, 0, 0, 0, 0, 0]) * self.LIMITED_SPEED
