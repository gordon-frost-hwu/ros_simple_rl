...
# def load_goals_by_type(load_type=None):
    # emotion_model_pkg_path = rospack.get_path('emotion_model')
    # f = open(emotion_model_pkg_path + '/src/emotion_model/default_goal_location.csv', 'r')
    # columns = f.readline()
    # columns = columns.strip('#\n').split(' ')
    # f.close()
    # goal_data = np.loadtxt(emotion_model_pkg_path + '/src/emotion_model/default_goal_location.csv', comments='#', delimiter=' ')
    # # GOAL_POSITIONS.append(list(goal_data))
    # if load_type is not None:
        # if load_type == "avoidance":
            # other_points_data = np.loadtxt(emotion_model_pkg_path + '/src/emotion_model/points_of_interest_avoidance.csv', comments='#', delimiter=' ')
        # elif load_type == "thruster":
            # other_points_data = np.loadtxt(emotion_model_pkg_path + '/src/emotion_model/points_of_interest_thruster.csv', comments='#', delimiter=' ')
        # elif load_type == "inspect":
            # other_points_data = np.loadtxt(emotion_model_pkg_path + '/src/emotion_model/points_of_interest_inspect.csv', comments='#', delimiter=' ')
    # else:
        # other_points_data = np.loadtxt(emotion_model_pkg_path + '/src/emotion_model/points_of_interest_default.csv', comments='#', delimiter=' ')
